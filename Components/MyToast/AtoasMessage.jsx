...
import { CircleAlert, CircleCheckBig, CircleX, MoveUpLeft, X } from 'lucide-react';
import { setMouseDownOnAToast } from './Trash';
const SuccessNotifSound = "/media/new-notification-017-352293.mp3"
const ErrorNotifSound = "/media/sample_error_fail01_kofi_by_miraclei-360129.mp3"
const delay = 6;
...
function playNotificationSound(
    ...
    const audio = new Audio(src ?? (sucess ? SuccessNotifSound : ErrorNotifSound));
    audio.volume = volume;
...
const MessageReadMore = ({ text }) => {
    return <p className=' ml-2 opacity-70   text-sm font-semibold'>{typeof (text) == "string" && text?.substring(0, 90)} {
    ...
    useEffect(() => {
        if (
            ...
            playNotificationSound({
                sucess: messageData.type2 == "success"
            })
        }
        ...
        <AnimatePresence>
            {
                ...
                <div
                    ...
                    <motion.div
                        ...
                        ref={cardRef}
                        className={`  ${isInDangerZone ? "bg-[#ff6e6e] " : " bg-background "} relative border-2 ${messageData.type2 == "success" ? "border-[#c3ffee]" : messageData.type2 == "error" ? "border-[#ef050547]" : "border-[#fffb2347]"}  w-full  font-medium rounded-xl  h-full flex items-center px-2  justify-center`}>
                        <div
                            style={{
                                filter: messageData.type2 == "success" ? "  drop-shadow(5px 0px 20px #54ff9bc6)" : messageData.type2 == "error" ? "drop-shadow(5px 0px 20px #ef050547)" : "drop-shadow(5px 0px 20px yellow)"
                            }}
                            className='p-[6px]   flex items-center   justify-center rounded-full  bg-[var(--bg-wh)]'>
                            {
                                messageData.type2 == "success"
                                    ? <CircleCheckBig className='w-7 h-7 stroke-2 text-[#00ffb3]' />
                                    : messageData.type2 == "error"
                                        ? <CircleX className='text-red-500' />
                                        : <CircleAlert  className='w-8 h-8 text-yellow-300'/>
                            }
