...
import { Ban, Check, CircleAlert, CircleCheck } from 'lucide-react';
...
const ExpandToast = ({ expandData, onClose }) => {
    ...
    const { title, message, type2, buttons, pop_it_directly } = expandData.messageData;
    ...
    return (
        <div className='fixed  duration-150 flex !z-[9999] items-center justify-center  w-full h-full top-0 left-0 bg-[#00000062]'>
            <motion.div
                ...
                <div className={`p-4  rounded-full ${type2 == "success" ? "bg-[#cfffea88]" : type2 == "error" ? "bg-[#ff959526]" : " bg-[#ffef1326] "}`}>
                    {
                        type2 == "success"
                            ? <Check className='w-14 h-14   bg-[#00ff91] p-2 rounded-full text-white stroke-3' />

                            : type2 == "error"
                                ? <Ban className='w-14 h-14   bg-[#ff2904] p-2 rounded-full text-white stroke-3' />
                                : <CircleAlert className='w-14 h-14   bg-[#f7eb0a] p-2 rounded-full text-white stroke-3' />
                    }
                </div>
                {
                    title && <h1 
                    className={`text-center tracking-tighter  text-2xl font-semibold ${type2 == "success" ? "" : type2 == "error" ? "text-[#c30000d6]" : " text-[#d1b906d6] "}`}>{title}</h1>
                }
                {
